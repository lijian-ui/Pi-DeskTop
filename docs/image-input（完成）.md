...
> 状态：**已实现**（2026-08-04）——粘贴 / 上传 / 拖拽三入口、图片胶囊预览、消息缩略图 + 点击大图、流式队列透传、历史回读还原
> 实现与本文档的差异：用户消息的图片**不是**靠乐观插入渲染，而是从 SDK `message_start` 事件与会话历史的 `image` content block 中解析（新增 `extractImages`），因此切换/重开会话后缩略图依然在。
